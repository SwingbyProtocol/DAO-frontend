...
class KpiRewardPoolContract extends Web3Contract {
  ...
  getMyDailyRewardFor(rewardTokenAddress: string): BigNumber | undefined {
    ...
    if (!balance || !this.poolSize) {
      return undefined;
    }

    if (this.poolSize.eq(BigNumber.ZERO)) {
  ...
  hasToClaim(): boolean {
    return Array.from(this.claims.values()).some(value => value.gt(0));
  }
...
export default KpiRewardPoolContract;
